// C:\DREAMAX\e-profile\material-kit-react\src\app\dashboard\customers\media\add\page.tsx
"use client";

import * as React from "react";
import { useRouter, useSearchParams } from "next/navigation";
import { addMedia, type MediaDTO } from "@/services/media.service";
...
function MediaForm({
	...
	const onSubmit = async (e: React.FormEvent) => {
		...
		setError(null);
		if (!athleteId) {
			setError("Thiếu thông tin vận động viên");
			return;
		}
		...
		try {
			...
			setError(err?.message || "Không thể lưu media");
		} finally {
			setSubmitting(false);
		}
	};
...
export default function Page() {
	...
	return <MediaForm mode="add" athleteId={Number.isFinite(athleteId) ? athleteId : undefined} title="Thêm media" />;
}
