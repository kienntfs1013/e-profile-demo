...
export function AchievementSection({ user }: { user?: User }): React.JSX.Element {
	...
	const onEditAchievement = (row: Achievement) =>
		router.push(
			...
			)}?athlete=${encodeURIComponent(user?.id || "")}&group=${encodeURIComponent(row.group)}&rank=${
				row.rank
			}&city=${encodeURIComponent(row.city)}&event=${encodeURIComponent(row.event)}&detail=${encodeURIComponent(
				row.detail
			)}&year=${row.year}`
		);
	...
	const rows = React.useMemo(() => {
		...
		return data;
	}, [q, group, sortBy, rankSort, achievements]);
	...
	return (
		<Stack spacing={3}>
					...
					<CardContent>
						...
						>
							<TextField
								fullWidth
								size="small"
								label="Tìm kiếm (giải / nội dung / thành phố / năm)"
								value={q}
									...
									{rows.map((r, i) => (
										<TableRow hover key={`${r.group}-${r.year}-${r.rank}-${i}`}>
											...
											<TableCell align="right">
												<IconButton size="small" aria-label="sửa" onClick={() => onEditAchievement(r)}>
													<PencilSimple />
												</IconButton>
												<IconButton size="small" aria-label="xóa" color="error" onClick={() => onDeleteAchievement(r)}>
													<Trash />
												</IconButton>
											</TableCell>
										</TableRow>
			...
			<Dialog open={!!confirm} onClose={() => setConfirm(null)} fullWidth maxWidth="xs">
				...
				<DialogContent>
					<DialogContentText>
						{confirm?.type === "ranking" && `Bạn có chắc muốn xóa mục xếp hạng “${confirm.payload.label}”?`}
						{confirm?.type === "medal" && `Bạn có chắc muốn xóa huy chương “${confirm.payload.label}”?`}
						{confirm?.type === "achievement" &&
							`Bạn có chắc muốn xóa thành tích ${confirm.payload.group} - ${confirm.payload.city} - ${confirm.payload.event} (${confirm.payload.year})?`}
					</DialogContentText>
				</DialogContent>
				<DialogActions>
