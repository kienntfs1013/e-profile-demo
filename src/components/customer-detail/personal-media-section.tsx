...
export function PersonalMediaSection({ id }: { id?: number | string }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const u =
          (await getUserById(targetId).catch(() => null)) ??
          (await fetchUserByIdFromList(targetId).catch(() => null));
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      setLoading(true);
      try {
        const data = await listMediaByAthlete(athleteId, "id-desc");
              ...
              pagedRows.map((r) => {
                ...
                return (
                  <TableRow key={r.id} hover>
                    <TableCell>
                      {r.thumbnail_url ? (
                        <Avatar variant="rounded" src={r.thumbnail_url} alt={r.title || ""} sx={{ width: 48, height: 48 }} />
                      ) : (
